...
#define DXGI_SWAPCHAIN_TRIPLE_BUFFERED
  ...
  int nHeight)
{
  ...
  //bitmapProps.pixelFormat.alphaMode = D2D1_ALPHA_MODE_STRAIGHT;
    ...
    VOID)
{
    ...
    desc.Width       = GetDeviceCaps(hDC, DESKTOPHORZRES);
    desc.Height      = GetDeviceCaps(hDC, DESKTOPVERTRES);

#if (defined DXGI_SWAPCHAIN_TRIPLE_BUFFERED)
    desc.BufferCount = 3;
#else
    desc.BufferCount = 2;
#endif
    ...
    desc.Flags = 
      DXGI_SWAPCHAIN_2_ENABLE_IMMEDIATE_PRESENT |
      DXGI_SWAPCHAIN_2_ENABLE_WAIT_FOR_NEXT_FRAME_RESOURCES;
    DeleteDC(hDC);
...
GetDxgiSwapchainSurfaceBitmapProperties(
    VOID)
{
    ...
    IDXGIImmersiveSwapchain* pImmersiveSwapchain)
{
    ...
    uRefCnt = IDXGIDevice_Release(pImmersiveSwapchain->pDXGIDevice1);
    //ASSERT_W32(!uRefCnt);
...

    {
      ASSERT_W32(SUCCEEDED(
        ID3D11Device_QueryInterface(
          pImmersiveSwapchain->pD3D11Device,
          &IID_IDXGIDevice1,
          (void**)&pImmersiveSwapchain->pDXGIDevice1
        )
      ));

      /* Since we're triple-buffered, set the target latency to be one frame. This reserves the right to cancel presents (and/or) not sequence them */
      IDXGIDevice1_SetMaximumFrameLatency(pImmersiveSwapchain->pDXGIDevice1, 0);
    }

    {
      const DXGI_SWAP_CHAIN_DESC1 c_desc = GetDxgiSwapchainDescription();
      const D2D1_FACTORY_OPTIONS  c_d2d_factory_options = { D2D1_DEBUG_LEVEL_INFORMATION };
      const D2D1_BITMAP_PROPERTIES1 c_bitmap_props = GetDxgiSwapchainSurfaceBitmapProperties();
      ...
      ASSERT_W32(SUCCEEDED(
        IDXGIFactory2_CreateSwapChainForComposition(
          ...
          (IUnknown*)pImmersiveSwapchain->pDXGIDevice1,
          &c_desc,
          0,
      ...
      ASSERT_W32(SUCCEEDED(
        ID2D1Factory2_CreateDevice1(
          pImmersiveSwapchain->pD2D1Factory2,
          pImmersiveSwapchain->pDXGIDevice1,
